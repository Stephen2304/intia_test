<div x-data="{ open: false }">
    ...
    <div x-show="open" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50" style="display: none;">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6">
            <h2 class="text-lg font-bold mb-4">Modifier la branche</h2>
            <form method="POST" action="{{ route('branches.update', $branch->id) }}">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-gray-200 mb-2">Nom</label>
                    <input type="text" name="name" value="{{ $branch->name }}" class="w-full border rounded px-3 py-2" placeholder="Nom de la branche">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-gray-200 mb-2">Localisation</label>
                    <input type="text" name="location" value="{{ $branch->location }}" class="w-full border rounded px-3 py-2" placeholder="Localisation">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 dark:text-gray-200 mb-2">Contact</label>
                    <input type="text" name="contact_info" value="{{ $branch->contact_info }}" class="w-full border rounded px-3 py-2" placeholder="Contact">
                </div>
