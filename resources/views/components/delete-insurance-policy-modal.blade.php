<div x-data="{ open: false }" class="inline">
    ...
    <div x-show="open" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50" style="display: none;">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md p-6">
            ...
            <p class="mb-4">Voulez-vous vraiment supprimer la police <h6 class="font-semibold">{{ $policy->policy_number }} ? </h6> </p>
            <form method="POST" action="{{ route('insurance_policies.destroy', $policy->id) }}">
                @csrf
                @method('DELETE')
